const Game = ({
	...
	const eventFunctions = map(f => e => { world = f(e)(world).run(); })(events);
	...
	const sendManual = e => IO(_ => void (world = manualEvent(e)(world).run()));
	const reset = IO(_ => void (world = initialWorld.run()));
	const getWorld = IO(_ => world);
	const setWorld = w => IO(_ => void (world = w));
	const updateWorld = f => IO(_ => void (world = f(world)));
