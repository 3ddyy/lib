...
IO.writeElem = elem => t => IO(_ => void (elem.innerHTML = t));
IO.writeValue = elem => v => IO(_ => void (elem.value = v));
IO.writeValueAsNum = elem => n => IO(_ => void (elem.valueAsNumber = n));
IO.writeDocument = t => IO(_ => document.write(t));
IO.writeStyle = prop => elem => s => IO(_ => void (elem.style[prop] = s));
...
IO.updateElem = elem => f => pipeK0( IO.readElem(elem), t => IO(_ => {
	const newT = f(t);
