import { map, of, pipeK, concatL } from "./category_theory.js";
import { toStr } from "./string.js";

export {
	Just, Nothing, Maybe,
	fromNullable, fromIsValid, isJust, isNothing,
	First, Last,
	safeDiv, path, maybe, getOrElse
};
...
const Just = x => ({
	...
	concat: y => y.isJust() ? Just(concatL(x)(y.getOrElse())) : Just(x),
	alt: _ => Just(x),
	isJust: _ => true,
	isNothing: _ => false,
	getOrElse: _ => x,
...
const getOrElse = d => m => m.getOrElse(d);