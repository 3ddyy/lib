...
const foldMap = M => f => pipe( map(f), fold(M) );
const convertFold = M => pipe( map(M), fold(M) );

const between = before => after => m => concatL(concatL(before)(m))(after);