...
import { eq, isObj, isF, isArr } from "./logic.js";
import { tail, drop, arrayFilterM, len } from "./array.js";
import {
	pipe, pipeWith, unCurry, nAry, mapf, apf, chainf, k, t, id, flip
} from "./function.js";
...
const liftArray = f => ms => {
	const arity = len(ms);
	return unCurry(liftN(arity)(nAry(arity)(f)))(...ms); //i think this is borked
};
