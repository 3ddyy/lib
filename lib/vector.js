...
import { mod, clamp, rndInt, rnd0Int } from "./number.js";
import { eq, xor } from "./logic.js";
import { pipe, curry } from "./function.js";
import { reject, lift, liftN, empty } from "./category_theory.js";
import { len, incl, eqAny, uniq } from "./array.js";
...
export {
	...
	wOf, hOf, sizeOf,
	vlen, vdiagonal,
	area, perimeter, volume, surface,
	dot, perpendicular, parallel,
	outside, inside,
	sRatio, toRatio, toSquare,
	toPos, toSize,
	putInside,
	edgeRect, edgeRects,
	edgeCuboid, edgeCuboids,
	trim,
	rndIntPos, rndIntPosExcept, midPos
};
...
Pos.UPLEFT    = Pos(-1,-1);
Pos.LEFT      = Pos(-1, 0);
Pos.DOWNLEFT  = Pos(-1, 1);
Pos.DOWN      = Pos( 0, 1);
Pos.DOWNRIGHT = Pos( 1, 1);
Pos.NONE      = Pos( 0, 0);
Pos.ALL = [
	Pos.RIGHT,
	Pos.UPRIGHT,
...
Rect.fromPosSize = ({x, y}, {w, h}) => Rect(x, y, w, h);
...
const Cuboid = (x, y, z, w, h, d) => ({
	...
	trim: margin => Cuboid(x, y, z, w, h, d).edgeCuboid(margin, empty(Pos3D)),
...
const sizeOf = lift(curry(Size))(wOf)(hOf);
...
const edgeCuboid = margin => dir => cuboid => cuboid.edgeCuboid(margin, dir);
const edgeCuboids = margin => dirs => cuboid => cuboid.edgeCuboid(margin, dirs);
