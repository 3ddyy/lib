const Pos = (x, y) => ({
	...
	putInside: (option, rect) => {
		if (option == "mod") return Pos(
			mod(rect.w)(x - rect.x),
			mod(rect.h)(y - rect.y)
		);
		if (option == "nearest_edge") return Pos(
			clamp(rect.x)(rect.x + rect.w)(x),
			clamp(rect.y)(rect.y + rect.h)(y)
		);
	},
...
const Size = (w, h) => ({
	...
	rndIntPosExcept: avoid_pos => {
		const pos = Size(w, h).rndIntPos();
		const avoid_pos_valid = pipe(uniq, reject(outside(rect)))(avoid_pos);
		if (len(avoid_pos_valid) >= w * h) return Nothing();
		if (incl(pos)(avoid_pos_valid))
			return Size(w, h).rndIntPosExcept(avoid_pos_valid);
		return Just(pos);
	},
...
const Rect = (x, y, w, h) => ({
	...
	rndIntPosExcept: avoid_pos => {
		const pos = Rect(x, y, w, h).rndIntPos();
		const avoid_pos_valid = pipe(uniq, reject(outside(rect)))(avoid_pos);
		if (len(avoid_pos_valid) >= w * h) return Nothing();
		if (incl(pos)(avoid_pos_valid))
			return Rect(x, y, w, h).rndIntPosExcept(avoid_pos_valid);
		return Just(pos);
	},
