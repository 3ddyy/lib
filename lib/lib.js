...
const pipeK = (...fs) => pipe(fs[0], pipeC(...dropFst(fs)));
const pipeK0 = (...m_fs) => pipe0(m_fs[0], pipeC(...dropFst(m_fs)));
const pipeconstK = (...ms) => pipe0(
	ms[0],
	pipeWith(constchain)(...dropFst(ms))
);
...
const percents = length => length > 1
	? fillArray(length).map((_, i) => i / (length - 1)) : [0];
