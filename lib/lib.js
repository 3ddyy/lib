...
const curryAdv = n => (...f) => (...x) => n > 1
	...
	: pipe(...f)(x);
const unCurryAdv = f => collect(reduce(spread)(f));
...
const dispatch = method => (...args) => (...options) => obj => {
	...
	if (obj.constructor[method]) return obj.constructor[method](...args)(obj);
...
const alli = f => pipe(mapi(f), reduce(and)(true));
const anyi = f => pipe(mapi(f), reduce(or)(false));
...
const filter = f => dispatch("filter")(f)(
	[isObj, o => filterk(x => _ => f(x))(o)]
);
...
const dropEnd = n => _lift(slice())(len, sub(n))(id);
...
const zipWith = f => xs => ys =>
	rep(min(len(xs))(len(ys)))(undefined).map((_, i) => f(xs[i])(ys[i]));
